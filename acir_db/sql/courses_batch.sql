-- Batch query template for aggregate_courses.py
-- Uses the full inner subquery from course_site_course.sql as the base so that
-- all deleted_at edge cases (course, course_site, site, org) are handled correctly.
--
-- LIMIT and OFFSET are injected by Python — do not run this file directly.
-- Only rows where deleted_at_info IS NULL are returned (fully active records).
--
-- Usage: sql = open("courses_batch.sql").read().format(limit=20000, offset=0)

SELECT
    c.course_id,
    c.course_name,
    c.course_standard_entry_requirements,
    c.course_alternate_entry_requirements,
    c.course_level_id,
    c.course_level_name,
    c.faculty_name,
    c.domestic_full_fee,
    c.overall_ielts_score,
    c.overall_toefl_score,
    c.free_tafe_course,
    c.organisation_id,
    c.organisation_name,
    c.course_site_id,
    csa.atar AS atar_value
FROM (
...
LEFT JOIN (
    SELECT csa1.course_site_id, csa1.atar
    FROM course_site_atar csa1
    INNER JOIN (
        SELECT course_site_id, MAX(year) AS max_year
        FROM course_site_atar
        GROUP BY course_site_id
    ) csa2 ON csa1.course_site_id = csa2.course_site_id
          AND csa1.year = csa2.max_year
) csa ON csa.course_site_id = c.course_site_id
